...
class DifferentialFlatnessNode(Node):
    def __init__(self):
        ...
        # Lets define internal variables Values From Mujoco
        self.g = 9.81
        self.mQ = 1.0 
        ...
        # Inertia Matrix  Values Obtained From Mujoco
        self.Jxx = 0.00305587
        self.Jyy = 0.00159695
        self.Jzz = 0.00159687
    ...
    def compute_reference(self, flag):
        ...
        if flag == 0 :
            hd, theta, hd_p, theta_p, hd_pp, hd_ppp, hd_pppp, theta_pp = self.ref_trajectory_agresive(10, a, b)
        ...
        for k in range(0, hd.shape[1]):
            ...
            # This is only to get the desired quaternion withou issues related to continuity
    ...
    def run(self):
        ...
        for k in range(0, self.t.shape[0]):
            ...
            #u[0, k], u[1:4, k] = self.position_control_flatness(self.x[0:3, k], self.h_d[0:3, k], self.x[3:6, k], self.h_d_d[0:3, k], self.h_d_dd[0:3, k], self.x[6:10, k], self.psi_d[k], self.x[10:13, k], self.w_d[:, k], self.w_d_d[:, k])
            u[0, k], u[1:4, k] = self.geometric_control(self.x[0:3, k], self.h_d[0:3, k], self.x[3:6, k], self.h_d_d[0:3, k], self.h_d_dd[0:3, k], self.x[6:10, k], self.psi_d[k], self.x[10:13, k], self.w_d[:, k], self.w_d_d[:, k])

            # Send Control Actions Mujoco
    ...
    def position_control_flatness(self, x, xd, x_d, xd_d, xd_dd, quat, psi, omega, omega_d, omega_d_d):
        kp = self.Kp
        kv = self.Kv
    ...
    def geometric_control(self, x, xd, v, v_d, a_d, quat, psi, omega, omega_d, omega_d_d):
        kp = self.Kp
        kv = self.Kv
        aux_variable = kp@(xd - x) + kv@(v_d - v) + a_d
        force_zb = self.mQ * (aux_variable + self.g*self.Zw[:, 0])
        ...
        # Coriolis
        c = np.cross(omega, self.J@omega)
        c = c.reshape((3, 1))

        # Angular velocity error
        omega_d = omega_d.reshape((3, 1))
        omega_d_d = omega_d_d.reshape((3, 1))
